...
Label(root, text ='Kunal', font = 'arial 20 bold').pack(side =BOTTOM)
...
def Mode():
    if(mode.get() == 'e'):
        Result.set(Encode(secret_key .get(), Text.get()))
    elif(mode.get() == 'd'):
        Result.set(Decode(secret_key .get(), Text.get()))
    else:
...
Entry(root, font = 'arial 10', textvariable = secret_key  , bg ='ghost white').place(x=290, y = 90)
